...
const QuizAssessment = ({ lesson, onComplete }) => {
  ...
  }, [answers, questions, timeLeft, timeLimit, onComplete])
  ...
  if (!currentQuestion) return <div className="p-8 text-center">No questions found.</div>
  ...
  return (
    <div className="h-[93vh] w-full bg-slate-50 relative overflow-hidden flex">
        ...
        <div className="space-y-4 p-8 flex flex-col justify-between h-full w-full max-w-2xl">
          ...
          <div className="flex justify-between items-center">
            ...
            <p className="font-extrabold text-2xl">
              {String(currentQuestionNum).padStart(2, '0')} of {String(totalQuestions).padStart(2, '0')}
            </p>
          ...
          <div className="w-full bg-slate-600 rounded-full h-4">
            <div className="bg-emerald-400 h-4 rounded-full transition-all"
          ...
          <div className="bg-white shadow p-6 mx-auto rounded-4xl space-y-4 border border-slate-200 w-full">
            ...
            <h1 className="text-3xl font-bold text-slate-700 text-center">{currentQuestion.text}</h1>
            <div className="space-y-4">
              {currentQuestion.options?.map(option => {
                ...
                return (
                  ...
                  >
                    <p className="text-slate-700 text-lg font-bold">{option.text}</p>
                  </div>
                )
              })}
            </div>
      ...
      <div className={`fixed inset-0 z-50 md:hidden transition-opacity duration-300 ${
        showQuestions ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setShowQuestions(false)} />
        <div className={`absolute right-0 top-0 h-full w-[85%] max-w-sm bg-slate-50 shadow-2xl p-6 overflow-y-auto transition-transform duration-300 ${
          ...
          <QuestionSidebar
            ...
            onSelectQuestion={(num) => { setCurrentIndex(num - 1); setShowQuestions(false) }}
            variant="mobile"
          />
        </div>
      </div>
