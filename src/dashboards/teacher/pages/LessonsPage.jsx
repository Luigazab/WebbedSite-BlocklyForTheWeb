...
import { Clipboard, PlusIcon, Search } from 'lucide-react'
...
export default function LessonsPage() {
  ...
  const [handOutTarget, setHandOutTarget] = useState(null)
  ...
  const load = async () => {
    setLoading(true)
    try {
      const data = await fetchTeacherLessons(user.id)
      setLessons(data || [])
    } catch (err) {
      console.error(err)
    } finally {
  ...
  const filtered = useMemo(() => {
    let list = lessons
    if (filter === 'published') list = list.filter((l) => l.is_published)
    if (filter === 'draft') list = list.filter((l) => !l.is_published)
    if (search.trim()) {
    ...
    <div>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-black text-slate-700">Class Materials</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((lesson) => (
              <LessonCard
                key={lesson.id}
                lesson={lesson}
                onEdit={(l) => navigate(`/teacher/lessons/${l.id}/edit`)}
                onDelete={(l) => setDeleteConfirm(l)}
                onHandOut={(l) => setHandOutTarget(l)}
              />
            ))}
          </div>
        )}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <h2 className="text-base font-bold text-slate-800 mb-2">Delete Lesson?</h2>
